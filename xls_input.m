...
% 1 - CHECK FILE
    if nargin == 0; filename = 'input/WetSnow/base.xlsx'; end
...
% 2 - EXTRACT DATA FROM FILE
 ...
 % 3 - APPLY SPECIAL VALUES TO ALBEDO AND EXTICTION COLUMNS
        A = albedo(A,atmTXT,S);
...
for i = 1:length(idx);
    ...
    if strcmpi('d',val(1)); % Optical depth caer
        ...
        [a1,S(idx(i),6),a2,S(idx(i),7)] = rad_calc(dopt,S(1,2));
    ...
    elseif length(val) > 5 && strcmpi('class',val(1:5));
        ...
        [a1,S(idx(i),6),a2,S(idx(i),7)] = rad_calc('class',cls);
...
for i = 1:size(S,1);
    ...
    if isnumeric(rho) && isnumeric(k); % Both specified
        S(i,2) = rho*1000; S(i,3) = k;
    elseif isnan(rho) && isnumeric(k); % Compute density
        S(i,2) = (log10(k) + 1.652) / 2.65 * 1000;
    elseif isnumeric(rho) && isnan(k); % Compute k
        if rho < 0.156;
